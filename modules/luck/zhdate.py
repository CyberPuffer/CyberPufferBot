...
class ZhDate():
    ...
    def chinese(self):
        ...
        if self.lunar_month == 1:
            ...
            zh_month += u"十{}".format(ZHNUMS[self.lunar_month - 10])
        zh_month += u'月'

        if self.lunar_day <= 10:
            zh_day = u'初{}'.format(ZHNUMS[self.lunar_day])
        elif self.lunar_day < 20:
            zh_day = u'十{}'.format(ZHNUMS[self.lunar_day - 10])
        elif self.lunar_day == 20:
            zh_day = u'二十'
        elif self.lunar_day < 30:
            zh_day = u'二十{}'.format(ZHNUMS[self.lunar_day - 20])
        else:
            zh_day = u'三十'

        year_tiandi = ZhDate.__tiandi(self.lunar_year - 1900 + 36) + '年'

        shengxiao = u"鼠牛虎兔龙蛇马羊猴鸡狗猪"
        
        return u"{}{}{} {} ({}年)".format(zh_year,zh_month,zh_day,year_tiandi,shengxiao[(self.lunar_year - 1900) % 12])
    ...
    def __str__(self):
        ...
        """
        if self.leap_month:
            return u"农历{}年闰{}月{}日".format(self.lunar_year, self.lunar_month, self.lunar_day)
        else:
            return u"农历{}年{}月{}日".format(self.lunar_year, self.lunar_month, self.lunar_day)
